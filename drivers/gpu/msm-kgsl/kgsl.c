...
int kgsl_device_platform_probe(struct kgsl_device *device)
{
	 ...
	 * nabu-v168: mainline GPU nodes use qcom,kgsl plus an iommus phandle.
	 * Do not require compatible=qcom,adreno; that string lets MSM DRM steal
	 * the device, and KGSL then returns -ENODEV for the same node.
	 */
	if (of_device_is_compatible(pdev->dev.of_node, "qcom,adreno") ||
	    of_property_present(pdev->dev.of_node, "iommus")) {
		status = kgsl_iommu_probe_standard(device, pdev);
		if (status) {
			dev_err(device->dev,
				"nabu kgsl: standard IOMMU probe failed: %d\n",
				status);
			goto error;
		}
	}
