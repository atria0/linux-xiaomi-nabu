...
static bool kgsl_iommu_uses_standard_bindings(struct kgsl_device *device)
{
	struct device_node *np;

	if (!device || !device->pdev)
		return false;
	np = device->pdev->dev.of_node;
	return of_device_is_compatible(np, "qcom,adreno") ||
		...
		struct kgsl_mmu *mmu, const struct io_pgtable_cfg *pgtbl_cfg)
{
	...
	if (kgsl_iommu_uses_standard_bindings(device))
...
static int kgsl_iopgtbl_alloc(struct kgsl_iommu_context *ctx, struct kgsl_iommu_pt *pt)
{
	...
	if (kgsl_iommu_uses_standard_bindings(ctx->kgsldev))
		...
		iommu_fault_handler_t handler, bool restore_drvdata)
{
	...
	if (IS_ERR_OR_NULL(context->domain)) {
		ret = (context->domain) ? PTR_ERR(context->domain) : -EPROBE_DEFER;
		if (ret == -ENODEV)
			ret = -EPROBE_DEFER;
...
int kgsl_iommu_probe_standard(struct kgsl_device *device, struct platform_device *pdev)
{
	...
	if (ret || of_genpd_add_device(&args, virt_dev)) {
		dev_info(&device->pdev->dev,
			 "nabu kgsl: skip SMMU genpd for fake CX device: %d\n",
			 ret);
